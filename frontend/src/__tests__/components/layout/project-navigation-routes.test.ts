...
describe("project navigation routes", () => {
  ...
  it("classifies freezone as xiahua and every production section as xiaji", () => {
    ...
    expect(projectModeFromPath("/projects/demo/anime")).toBe("xiaji");
    expect(projectModeFromPath("/projects/demo/tasks")).toBe("xiaji");
  });

  it("registers Anime Studio as a first-class project section", () => {
    expect(PROJECT_SECTION_ROUTES.anime).toBe("/projects/$project/anime");
    expect(projectSectionFromPath("/projects/demo/anime")).toBe("anime");
  });
